...
#                 self.history['value_losses'].append(value_loss.detach().cpu().item())
    ...
    def learn(self):
        for iteration in range(self.args['num_iterations']):
            ...
            actors = [SelfPlayRay.options(num_cpus=1 / distributed_num, num_gpus=1 / distributed_num).remote(self.model, self.game, self.args, self.monitor) for i in range(distributed_num)]
