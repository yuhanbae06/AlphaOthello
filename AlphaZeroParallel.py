...
@ray.remote(gpus=0.2)
class SelfPlayRay:
    def __init___(self, model, game, args, monitor):
        self.model = model
        self.game = game
        self.args = args
        self.mcts = MCTSParallel(game, args, model)
...
class AlphaZeroParallelRay:
    ...
    def learn(self):
        for iteration in range(self.args['num_iterations']):
            ...
            actors = [SelfPlayRay.remote(self.model, self.game, self.args, self.monitor) for i in range(self.args['num_selfPlay_iterations'] // self.args['num_parallel_games'])]
