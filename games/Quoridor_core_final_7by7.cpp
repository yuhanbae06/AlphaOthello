...
public:
    ...
    static const int NUM_SQUARES = SIZE * SIZE;            // 49
    static const int WALL_SIZE = SIZE + 1;                 // 8
    static const int WALL_CNT = WALL_SIZE * WALL_SIZE;     // 64
    static const int ACTION_SIZE = 4 + 2 * WALL_CNT + 1;
            ...
            while (candidates) {
                ...
                if (!overlap_v) {
                    ...
                    if (!is_bridge_fast(state, w_r, w_c, 1) || (has_path(state, 0) && has_path(state, 1))) {
                        moves_out[count++] = (4 + WALL_CNT + idx);
                    }
...
int main() {
    ...
    static const int REP_NUM = 10000;
    ...
    for (int i = 0; i < REP_NUM; i++) {
        ...
        int moves[Quoridor::ACTION_SIZE] = {};
        int num = 0;
        ...
        while (true) {
            ...
            if (engine.check_win(state, 0)) break;
            if (engine.check_win(state, 1)) break;

            int move_count = engine.get_valid_moves(state, moves);
            if (move_count == 0) {
                std::cout << "NO MOVES " << state.is_jumping << ' ' << (int)state.jumper_idx << ' ' << (int)state.jump_dir << '\n'; // u d l r
                engine.render(state);
                break;
            }

            int random_move = moves[std::rand() % move_count];
...
int main() {
    ...
    std::cout << "=== STARTING VISUAL TEST ===\n";
    State state = engine.get_initial_state();
    
    // 1. Render the initial empty board
    engine.render(state);

    int moves[Quoridor::MAX_ACTIONS] = {};
    int turn_count = 0;

    while (true) {
        turn_count++;

        // 2. Check for winners BEFORE generating moves
        if (engine.check_win(state, 0)) {
            std::cout << "\n🏆 Player 1 (1) Wins in " << turn_count << " turns!\n";
            break;
        }
        if (engine.check_win(state, 1)) {
            std::cout << "\n🏆 Player 2 (2) Wins in " << turn_count << " turns!\n";
            break;
        }

        int move_count = engine.get_valid_moves(state, moves);
        if (move_count == 0) {
            std::cout << "\n⚠️ No valid moves left. Game stuck!\n";
            break;
        }

        // 3. Pick a random move and apply it
        int random_move = moves[std::rand() % move_count];
        state = engine.apply_action(state, random_move);

        // 4. Render the board after the move
        std::cout << "\n====================================\n";
        std::cout << "Turn " << turn_count << " (Player " << (state.turn == 0 ? "2" : "1") << " just moved)\n";
        std::cout << "====================================\n";
        engine.render(state);

        // 5. PAUSE so you can look at the board
        // Option A: Wait for 0.5 seconds automatically
        std::this_thread::sleep_for(std::chrono::milliseconds(500)); 
        
        // Option B: Wait for you to press ENTER (uncomment the line below if you prefer this)
        // std::cin.get(); 
    }
